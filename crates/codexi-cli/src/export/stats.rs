// src/export/stats.rs

use anyhow::Result;
use minijinja::{Environment, context};
use rust_decimal::Decimal;
use thousands::Separable;

use codexi::{core::format_id_short, dto::StatsCollection};

const STATS_TEMPLATE: &str = include_str!("../assets/templates/stats.html");

pub fn export_stats_html(entry: StatsCollection) -> Result<String> {
    let mut env = Environment::new();
    env.add_template("stats.html", STATS_TEMPLATE)?;

    // Savings bar : clamp 0–100 for the bar width, flag negative for color

    let savings_rate_negative = match entry.savings_rate {
        Some(v) => v < Decimal::ZERO,
        None => false,
    };

    let savings_bar_pct = entry
        .savings_rate
        .map(|v| v.max(Decimal::ZERO).min(Decimal::ONE_HUNDRED).to_string())
        .unwrap_or_else(|| "0".to_string());

    let savings_value = entry
        .savings_rate
        .map(|v| format!("{:.2}", v))
        .unwrap_or_else(|| "N/A".to_string());

    let top_expenses: Vec<serde_json::Value> = entry
        .top_expenses
        .iter()
        .map(|exp| {
            serde_json::json!({
                "op_id": format!("#{}", format_id_short(&exp.op_id)),
                "op_date": exp.op_date,
                "description": exp.description,
                "amount": format!("{:.2}", exp.amount).separate_with_commas(),
                "percentage": format!("{:.1}", exp.percentage),
            })
        })
        .collect();

    let ignored: Vec<serde_json::Value> = entry
        .ignored
        .iter()
        .map(|op| {
            serde_json::json!({
                "op_id": format!("#{}", format_id_short(&op.id)),
                "op_date": op.date,
                "type": op.flow,
                "amount": format!("{:.2}", op.amount).separate_with_commas(),
                "description": op.description,
            })
        })
        .collect();

    let ctx = context! {
        // Overview
        date_min => entry.from.unwrap_or_else(|| "N/A".to_string()),
        date_max => entry.to.unwrap_or_else(|| "N/A".to_string()),
        total_credit => format!("{:.2}", entry.total_credit).separate_with_commas(),
        total_debit => format!("{:.2}", entry.total_debit).separate_with_commas(),
        balance => format!("{:.2}", entry.balance).separate_with_commas(),
        operation_count => entry.operation_count,
        average_operation => format!("{:.2}", entry.average_operation).separate_with_commas(),
        daily_average => format!("{:.2}", entry.daily_average).separate_with_commas(),

        // Savings bar
        savings_rate => savings_value,
        savings_rate_negative => savings_rate_negative,
        savings_bar_pct => savings_bar_pct,

        // Insights
        max_single_debit => format!("{:.2}", entry.max_single_debit).separate_with_commas(),
        adjustment_count => entry.adjustment_count,
        adjustment_percentage => format!("{:.1}", entry.adjustment_percentage),
        days_count => entry.days_count,

        // Collections
        top_expenses => top_expenses,
        ignored => ignored,
    };

    let template = env.get_template("stats.html")?;
    let html = template.render(ctx)?;

    Ok(html)
}
