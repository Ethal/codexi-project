...
pub fn view_balance(balance: &BalanceItem) {
    ...
    let credit_value = CREDIT_STYLE.apply_to(format!("{:.2}", balance.credit).separate_with_commas());
    let debit_value = DEBIT_STYLE.apply_to(format!("{:.2}", balance.debit).separate_with_commas());
    let balance_value = VALUE_STYLE.apply_to(format!("{:.2}", balance.total).separate_with_commas());
