...
use crate::ui::{NOTE_STYLE, TITLE_STYLE, format_long_id_to_short};
...
pub fn view_account(items: &AccountEntry) {
    ...
    for item in items.items.iter() {
        ...
        println!(
            " {} {} {} - {} {} {} {}",
            marker, item.id, format_long_id_to_short(&item.id), item.name, item.context.account_type, item.currency, item.bank
        );
    }
    println!();
    println!(
...
pub fn view_account_context(item: &AccountItem) {
    ...
    println!();
    println!("{}({} {}) {}", title_text_1, item.name, item.currency, title_text_2);
